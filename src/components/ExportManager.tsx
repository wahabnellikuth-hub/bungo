...
export const ExportManager: React.FC = () => {
  ...
  return (
    <section>
      <h3 className="text-lg font-bold text-stone-800 mb-3">Export Data</h3>
      <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 text-center">
        <p className="text-sm text-stone-600 mb-4 font-medium">Download all order history as a CSV file for Excel or Google Sheets.</p>
        <button 
          onClick={handleExport}
          className="w-full flex items-center justify-center px-4 py-3 bg-stone-800 text-white font-bold rounded-xl hover:bg-stone-900 transition-colors shadow-sm mb-6"
        >
          <Download size={20} className="mr-2" /> Export Orders
        </button>

        <div className="border-t border-stone-200 pt-6 mt-2">
          <p className="text-sm text-red-600 mb-4 font-medium">Danger Zone: Wipe all data to start fresh for a new day.</p>
          <button 
            onClick={() => {
            ...
            className="w-full flex items-center justify-center px-4 py-3 bg-red-50 text-red-700 font-bold rounded-xl hover:bg-red-100 border border-red-200 transition-colors"
          >
